...
void main() {
  testWidgets('StoreProvider passes a Redux Store down to its ancestors',
  ...
  testWidgets('StoreConnector can convert the store to a ViewModel',
      (WidgetTester tester) async {
    final initial = "initial";
    final widget = new StoreProvider(
      store: new Store(new IdentityReducer(), initialState: initial),
  ...
  testWidgets(
      'StoreConnector builds the latest state of the store after a change event',
      (WidgetTester tester) async {
    final initial = "initial";
  ...
  testWidgets(
      'StoreConnector rebuilds by default whenever the store emits a change',
      (WidgetTester tester) async {
    int numBuilds = 0;
    final initial = "initial";
    final store = new Store(
  ...
  testWidgets(
      'StoreConnector avoids rebuilds when distinct is used with an object that implements ==',
      (WidgetTester tester) async {
    int numBuilds = 0;
    final initial = "initial";
    final store = new Store(
